...
const updateRetailerProduce = asyncHandler(async (req,res)=>{
  res.header("Access-Control-Allow-Origin","*")
  const {newFarmer} = req.body
 console.log('WE ARE IN BACKEND FOR THE ONE AND ONLY RETAILER PRODUCE-->',req.body)
  const objectId = new mongoose.Types.ObjectId(req.body._id)
  const product= await RetailerProduct.findById(objectId)
   ...
   if(product){
    ...
    product.image = req.body.leadFile?req.body.leadFile:product.image
    product.availableOnCredit = req.body.availableOnCredit?req.body.availableOnCredit:product.availableOnCredit
    product.imageUrls = req.body.imageUrls?req.body.imageUrls:product.imageUrls
...
//@desc  Get all products for the request in focus
//@route GET /api/retailer-products/requestinfocus
//@access Public

const getRetailerProductsForRequestInFocus = asyncHandler(async (req, res) => {
  res.header("Access-Control-Allow-Origin", "*");

  console.log("WE ARE IN RETAILER PRODUCT CONTROLLER",req.body.productIdArray);

  const objectIdArray = req.body.productIdArray;

  // Validate input
  if (!Array.isArray(objectIdArray) || objectIdArray.length === 0) {
    return res.status(400).json({ message: "productIdArray must be a non-empty array." });
  }
  console.log("WE ARE BOUT TO CONVERT TO OBJECT IDS");
  // Convert to ObjectId instances (in case they come as strings)
  const convertedIds = objectIdArray.map(id => new mongoose.Types.ObjectId(id));

  // Find all RetailerProducts where _id is in objectIdArray
  const products = await RetailerProduct.find({ _id: { $in: convertedIds } });
...
export {addNewRetailerProduct,getRetailerProductsForRequestInFocus,getProducts, getProductById,deleteProduct, createProduct,addProduce,updateRetailerProduce, updateProduct, updateProductStockCount, createProductReview ,getTopProducts,getAllRetailerProducts,getAllRetailerProductsPeriod}
